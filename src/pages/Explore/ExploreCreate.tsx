...
export default function ExploreCreatePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (imageFiles.length === 0) {
      alert('최소 1장 이상의 전시 이미지를 업로드해주세요.')
      return
    }
    // 등록 완료 처리 -> 완료 모달 노출
    setIsCompleted(true)
        ...
        >
          <div className="border-b border-white/10" style={{ paddingBottom: '24px' }}>
            ...
            <p className="text-gray-400 text-sm" style={{ marginTop: '8px' }}>
              새로운 기획전시나 문화 예술 콘텐츠 정보를 등록해주세요.
            </p>
          </div>

          <form onSubmit={handleSubmit} className="flex flex-col" style={{ gap: '28px' }}>
            <div className="flex flex-col" style={{ gap: '10px' }}>
              <label className="text-xs font-semibold text-gray-300">전시 이름</label>
              <input
                required
                type="text"
                name="name"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: '20px' }}>
              <div className="flex flex-col" style={{ gap: '10px' }}>
                <label className="text-xs font-semibold text-gray-300">장소 / 위치</label>
                <input
                  required
                  type="text"
                  name="location"
                  ...
                  onChange={handleChange}
                  placeholder="예) 서울 성동구 연무장길 20"
                  className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 transition"
                  style={{ padding: '16px 18px' }}
                />
              </div>
              <div className="flex flex-col" style={{ gap: '10px' }}>
                <label className="text-xs font-semibold text-gray-300">카테고리</label>
                <input
                  required
                  type="text"
                  name="category"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: '20px' }}>
              <div className="flex flex-col" style={{ gap: '10px' }}>
                <label className="text-xs font-semibold text-gray-300">전시 일정</label>
                <input
                  required
                  type="text"
                  name="period"
                  ...
                  onChange={handleChange}
                  placeholder="예) 2026.05.01 - 2026.06.30"
                  className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 transition"
                  style={{ padding: '16px 18px' }}
                />
              </div>
              <div className="flex flex-col" style={{ gap: '10px' }}>
                <label className="text-xs font-semibold text-gray-300">운영 시간</label>
                <input
                  required
                  type="text"
                  name="operatingHours"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2" style={{ gap: '20px' }}>
              <div className="flex flex-col" style={{ gap: '10px' }}>
                <label className="text-xs font-semibold text-gray-300">관람 요금</label>
                <input
                  required
                  type="text"
                  name="ticketPrice"
                  ...
                  onChange={handleChange}
                  placeholder="예) 성인 18,000원 / 무료"
                  className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 transition"
                  style={{ padding: '16px 18px' }}
                />
              </div>
              <div className="flex flex-col" style={{ gap: '10px' }}>
                <label className="text-xs font-semibold text-gray-300">주관 / 큐레이터</label>
                <input
                  required
                  type="text"
                  name="curator"
            ...
            <div className="flex flex-col" style={{ gap: '10px' }}>
              <label className="text-xs font-semibold text-gray-300">주요 특징 (쉼표로 구분)</label>
              <input
                type="text"
                name="highlights"
                value={formData.highlights}
                onChange={handleChange}
                placeholder="예) 인터랙티브 미디어, 포토존 다수, 오디오 가이드 제공"
                className="w-full bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 transition"
                style={{ padding: '16px 18px' }}
              />
            </div>
            ...
            <div className="flex flex-col" style={{ gap: '10px' }}>
              <label className="text-xs font-semibold text-gray-300">전시 소개</label>
              <textarea
                required
                rows={5}
                name="description"
                ...
                onChange={handleChange}
                placeholder="전시 기획 의도 및 관람 포인트를 상세히 적어주세요."
      ...
      {isCompleted && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#131924] border border-white/10 rounded-3xl max-w-md w-full p-8 shadow-2xl flex flex-col items-center text-center animate-in fade-in zoom-in duration-200">
            <h2 className="text-2xl font-bold text-white mb-2" style={{ marginBottom: '24px' }}>
              전시 등록 신청 완료
            </h2>
            <p
            ...
            >
              제출해주신 전시 정보는 영업일 기준 <strong className="text-indigo-400">3일 내</strong>
